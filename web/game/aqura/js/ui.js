...
export class UIManager {
  ...
  bindEvents() {
    ...
    this.dom.soundBtn.addEventListener("click", () => {
      ...
      const label = this.dom.soundBtn.querySelector("span");
      if (label) label.textContent = isMuted ? "静音" : "声音";
      const svg = this.dom.soundBtn.querySelector(".icon-svg");
      if (svg) {
        if (isMuted) {
  ...
  updateStatsDisplay() {
    ...
    // Keep active fish card metrics updated in real-time
    if (this.selectedFishInstance && !this.dom.fishCard.classList.contains("is-hidden")) {
      const f = this.selectedFishInstance.data;
      if (f) {
